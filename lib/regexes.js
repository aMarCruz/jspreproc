...
var RE = module.exports = {
  ...
  CCCOMMS: /^[ \t]*\/\/#[ \t]*(if(?:n?def)?|el(?:if|se)|endif|define|undef|set|unset|include(?:_once)?)(?=[ \t\(\n]|$)([^\n]*)\n?/g,
  // Multi-line comment
  MLCOMMS: /\/\*[^*]*\*+(?:[^*\/][^*]*\*+)*\//g,
  // Single-line comment
  SLCOMMS: /\/\/[^\n]*$/gm,
  // Quoted strings, this don't care about embedded eols
  STRINGS: /"(?:[^"\\]*|\\[\S\s])*"|'(?:[^'\\]*|\\[\S\s])*'/g,
...
var REPVARS = new RegExp(RE.S_QBLOCKS + '|([^$\\w])(\\$_[_0-9A-Z]+)\\b', 'gm')
