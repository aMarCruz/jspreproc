 ...
 */
ccp.RE = /^[ \t]*\/\/#[ \t]*(if(?:n?def)?|el(?:if|se)|endif|define|undef|set|unset|include(?:_once)?)(?=[ \t\(\n]|$)([^\n]*)\n?/
...
ccp.CC = function (file, level, opts) {
  ...
  varset.__FILE = this.fname   // relative path

  // Read and evaluate the header for this file
  var hdr = opts[level > 0 ? 'headers' : 'header1']
  if (hdr)
    this.header = evalExpr(hdr, varset, opts.emitError)
...
ccp.reset = function reset(old, cc) {
  ...
  if (old.block[1])
    ...
    varset.__FILE = cc ? cc.fname : ''

  return cc
}

/**
 * Replaces varset that begins with '$_' through all the code.
...
ccp.redef = function redef(str) {
  ...
  function rep(m, x, v) {
    return v in varset ? x + varset[v] : m  // don't replace undefined names
  }
