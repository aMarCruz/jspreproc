...
function ccp(key, expr, cc) {
  ...
  if (!expr && key !== 'else' && key !== 'endif')
    throw new Error('Expected expression for #' + key)
...
function getValue(expr, ckey) {
  ...
  if (ckey !== 'if') {
    ...
    return expr in options.defines ? yes : (yes ^ 1)
  }
 ...
 *
 * @param   {string} file - Filename of current file
 * @param   {Object} opts - User options
 * @returns {Object} A new configuration object
 */
ccp.CC = function CC(file, level, opts) {
  var
    cc = {
      output: true,
      last:   0,
      ...
      level:  level,
      file:   file,
      _file:  file ? require('path').relative('.', file).replace(/\\/g, '/') : ''
    },
    ids, hdr

  // Keep local reference to the global options object
  options = opts
  options.defines.__FILE = cc._file             // relative path

  // Read and evaluate the header for this file
  ids = level > 0 ? 'headers' : 'header1'
  if (hdr = opts[ids]) {
    if (hdr[0] !== "'" && hdr[0] !== '"')
      hdr = opts[ids] = '"' + hdr.replace(/"/g, '\\"') + '"'
    cc.header = evalExpr(hdr, opts)
  }
...
/**
 * Restore the predefined symbol `__FILE`.
 * This avoids setting the value in each call to `evalExpr`.
 *
 * @param   {Object} cc - The configuration object
 * @returns {Object} The same configuration object, with `__FILE` updated
 */
ccp.reset = function reset(cc) {
  options.defines.__FILE = cc._file
  return cc
}
...
ccp.redef = function redef(str) {
  if (~str.indexOf('$_')) {
    ...
    str = str.replace(REPVARS, function (m, x, v) {
