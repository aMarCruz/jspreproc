  ...
  while (match = HTML_ATTR.exec(str)) {
    ...
    if (!v)
      ...
      if (k === 'type' && v.toLowerCase() === '"number"') {
        v = ID_NUMBER   // fix #827 by @rsbondi
      }
...
/*
  Removes comments, replaces the expressions with a marker for easier analysis,
 ...
 * @param   { Object } opts   - Collected user options. Includes the brackets array.
...
function riotjs(js) {
  ...
  // Find the position following the closing bracket
...
/**
 * Runs the parser for the js code, defaults to riotjs
 *
...
function getCode(code, opts, attrs) {
  ...
  if (src) {
    var chset = getAttr(attrs, 'charset'),
        path = require('path'),
        fs = require('fs')
    attrs = path.resolve(opts.url || opts.basedir || '.', src)
...
//# CUST_TAG don't allow unquoted expressions containing the `>` operator
var
  CUST_TAG = /^[ \t]*<([-\w]+)\s*([^'"\/>]*(?:(?:\/[^>]|"[^"]*"|'[^']*')[^'"\/>]*)*)(?:\/|>\n?([^<]*(?:<(?!\/\1\s*>)[^<]*)*)<\/\1\s*)>[ \t]*/gim,
  STYLE = /<style(\s+[^>]*)?>\n?([^<]*(?:<(?!\/style\s*>)[^<]*)*)<\/style\s*>/gi,
